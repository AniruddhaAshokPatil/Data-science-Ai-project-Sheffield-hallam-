...
def hash_password(password: str, salt: str | None = None) -> tuple[str, str]:
    # PBKDF2 stores a one-way password hash instead of keeping the real password in the database.
...
def decode_access_token(token: str) -> AuthenticatedUser:
    try:
        ...
        raise HTTPException(status_code=status.HTTP_401_UNAUTHORIZED, detail="The access token could not be parsed.") from exc
    ...
    if not hmac.compare_digest(signature, expected_signature):
        raise HTTPException(status_code=status.HTTP_401_UNAUTHORIZED, detail="The access token could not be verified.")
    ...
    if expires_at < datetime.now(timezone.utc):
        raise HTTPException(status_code=status.HTTP_401_UNAUTHORIZED, detail="The access token has expired.")
...
def _extract_bearer_token(authorization: str | None) -> str:
    if not authorization:
        raise HTTPException(status_code=status.HTTP_401_UNAUTHORIZED, detail="An Authorization header is required.")
...
def require_user_role(current_user: AuthenticatedUser = Depends(get_current_user)) -> AuthenticatedUser:
    if current_user.role not in {"user", "investigator"}:
        raise HTTPException(status_code=status.HTTP_403_FORBIDDEN, detail="This user role is not authorized.")
    return current_user


def require_customer_role(current_user: AuthenticatedUser = Depends(get_current_user)) -> AuthenticatedUser:
    if current_user.role != "user":
        raise HTTPException(status_code=status.HTTP_403_FORBIDDEN, detail="A policyholder account is required for claim submission.")
    return current_user


def require_investigator_role(current_user: AuthenticatedUser = Depends(get_current_user)) -> AuthenticatedUser:
    if current_user.role != "investigator":
        raise HTTPException(status_code=status.HTTP_403_FORBIDDEN, detail="An investigator account is required for this action.")
    return current_user


def get_websocket_user(token: str | None = Query(default=None)) -> AuthenticatedUser:
    if not token:
        raise HTTPException(status_code=status.HTTP_401_UNAUTHORIZED, detail="A token query parameter is required.")
    return decode_access_token(token)
