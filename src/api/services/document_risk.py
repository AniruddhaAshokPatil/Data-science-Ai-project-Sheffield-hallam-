...
DUPLICATE_FILE_RISK_WEIGHT = 0.34
SMALL_IMAGE_RISK_WEIGHT = 0.24
EXTREME_ASPECT_RATIO_RISK_WEIGHT = 0.14
LOW_COLOUR_MODE_RISK_WEIGHT = 0.10
UNREADABLE_IMAGE_RISK_WEIGHT = 0.42
PDF_BASE_RISK_WEIGHT = 0.10
SMALL_PDF_RISK_WEIGHT = 0.20
...
async def analyze_and_store_evidence(upload: UploadFile | None) -> dict:
    if upload is None or not upload.filename:
        return {
            ...
            "image_tamper_flag": 0,
            "document_risk_score": RECEIPT_MISSING_BASE_RISK,
    ...
    document_risk_score = RECEIPT_PRESENT_BASE_RISK
    image_tamper_flag = 0
    summary_parts = [f"The uploaded evidence was stored as {stored_filename}."]
    analysis_reasons = []

    if duplicate_match:
        document_risk_score += DUPLICATE_FILE_RISK_WEIGHT
    ...
    document_risk_score = round(min(document_risk_score, DOCUMENT_RISK_CAP), 2)
    try:
...
def _score_image_evidence(file_bytes: bytes) -> dict:
    try:
        ...
        if min(width, height) < 300:
            risk_delta += SMALL_IMAGE_RISK_WEIGHT
        ...
        if aspect_ratio > 4:
            risk_delta += EXTREME_ASPECT_RATIO_RISK_WEIGHT
        ...
        if image.mode in {"1", "P"}:
            risk_delta += LOW_COLOUR_MODE_RISK_WEIGHT
        ...
        return {
            ...
            "image_tamper_flag": int(risk_delta >= SMALL_IMAGE_RISK_WEIGHT),
            "summary": " ".join(summary_parts),
            "reasons": reasons or ["Receipt image uploaded and passed the basic visual checks."],
        }
    except (UnidentifiedImageError, OSError):
        return {
            "risk_delta": UNREADABLE_IMAGE_RISK_WEIGHT,
            "image_tamper_flag": 1,
