...
def claim_history_row(email_row, index, scenario_name, risk_label, review_outcome, evidence_key, evidence_path):
    ...
    return {
        ...
        "claimant_id": email_row["claimant_id"],
        "policy_type": "gadget_equipment",
        "coverage_tier": email_row["coverage_tier"],
        "policy_start_date": "2024-09-01",
        "claim_date": email_row["incident_date"],
        "claim_submission_timestamp": f"{email_row['incident_date']}T10:{index + 10:02d}:00",
        "claim_channel": email_row["claim_channel"],
        "claimant_age_band": ["18-24", "25-34", "35-44", "45-54"][index % 4],
        "claimant_tenure_days": 40 + (index * 37),
        "postal_region": email_row["customer_region"],
        "item_category": email_row["item_category"],
        "item_purchase_date": email_row["purchase_date"],
        "claimed_incident_date": email_row["incident_date"],
        "incident_type": email_row["incident_type"],
        "claim_amount_gbp": email_row["claim_amount_gbp"],
        "estimated_item_value_gbp": email_row["purchase_price_gbp"],
        "claim_amount_vs_item_value_ratio": round(float(email_row["claim_amount_gbp"]) / float(email_row["purchase_price_gbp"]), 2),
        "prior_claims_count": 3 if high_risk else (1 if medium_risk else 0),
        "claims_last_12_months": 2 if high_risk else (1 if medium_risk else 0),
        ...
        "days_since_policy_start": 18 if high_risk else (95 if medium_risk else 260),
        "premium_payment_missed_last_12_months": int(high_risk),
        "recent_high_value_purchase_flag": int(medium_risk or high_risk),
        "unusual_spend_spike_flag": int(high_risk),
        "account_login_location_change_flag": int(high_risk),
        "multiple_devices_last_7_days_flag": int(high_risk),
        "address_change_last_30_days_flag": int(high_risk),
        "phone_change_last_30_days_flag": int(medium_risk or high_risk),
        "bank_detail_change_last_30_days_flag": int(email_row["bank_change_flag"]),
        "late_night_submission_flag": int(high_risk),
        "weekend_submission_flag": int(index % 3 == 0),
        "receipt_present_flag": receipt_present,
        "receipt_mismatch_flag": receipt_mismatch,
        "duplicate_receipt_flag": duplicate_receipt,
        "image_tamper_flag": image_tamper,
        ...
        "insurer_id": email_row["insurer_brand"],
        "policy_number": email_row["policy_number"],
        "claim_status": "under_review" if review_outcome == "flagged" else "approved",
        "customer_segment": email_row["customer_segment"],
        "payment_method": "direct_debit",
        "reported_loss_location": email_row["reported_loss_location"],
        "police_report_flag": email_row["evidence_police_report_flag"],
        "known_fraud_history_flag": int(high_risk),
        "claim_description_length": len(email_row["body"]),
        "settlement_amount_gbp": 0 if review_outcome == "flagged" else round(float(email_row["claim_amount_gbp"]) * 0.82, 2),
        "payout_method": email_row["payment_preference"],
        ...
        "imei_or_serial_present_flag": email_row["serial_or_imei_present_flag"],
        "evidence_count": int(bool(evidence_path)) + int(email_row["evidence_repair_quote_flag"]) + int(email_row["evidence_photo_flag"]),
        ...
        "excess_amount_gbp": email_row["excess_amount_gbp"],
